...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO bookings(RideID, DriverID, DepartureLocation, Destination, DepartureTime,AvailableSeats) VALUES (?, ?, ?, ?, ?,?)");
    $stmt->bind_param("isssss", $RideID, $DriverID, $DepartureLocation, $Destination, $DepartureTime,$AvailableSeats);
    // Set parameters and execute
    $RideID = $_POST['book_id'];
    $DriverID = $_POST['ride_id'];
    $DepartureLocation = $_POST['passenger_id'];
    $Destination = $_POST['booking_time'];
    $DepartureTime = $_POST['bookingtime'];
    $AvailableSeats = $_POST['bookintime'];
...
<body>
    <center><h2>Rides Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all rides
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $RideID = $row['RideID']; // Fetch the RideID
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_rides.php?RideID=$RideID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_rides.php?RideID=$RideID'>Update</a></td> 
        </tr>";
    }
