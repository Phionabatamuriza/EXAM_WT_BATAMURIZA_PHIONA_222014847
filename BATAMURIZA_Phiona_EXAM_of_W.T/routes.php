...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO routes(RouteID, RideID, Location) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $RouteID, $RideID, $Location);
    // Set parameters and execute
    $RouteID = $_POST['book_id'];
    $RideID = $_POST['ride_id'];
    $Location = $_POST['passenger_id'];
...
<body>
    <center><h2>Routes Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all routes
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $RouteID = $row['RouteID']; // Fetch the RouteID
        echo "<tr>
       
            <td>" . $row['RouteID'] . "</td>
            <td>" . $row['RideID'] . "</td>
            <td>" . $row['Location'] . "</td>
            <td><a style='padding:4px' href='delete_routes.php?RouteID=$RouteID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_routes.php?RouteID=$RouteID'>Update</a></td> 
        </tr>";
    }
